...
function HomePage() {
  ...
  const fetchTasksForMonth = useCallback(async (month: number, year: number) => {
    setLoading(true)
    try {
  ...
  const handleProcessTasks = async (task: any) => {
  ...
  // Function to handle create task from natural language
  const handleCreateTaskFromNL = async (task: TaskExtract) => {
    try {
      console.log("Creating task from natural language input:", task);
      ...
      // Make sure scrolling is re-enabled even on error
      document.body.style.overflow = '';
    }
  };

  // Handle tasks from natural language extraction
  const handleCreateTasksFromExtract = async (extractedTasks: TaskExtract[]) => {
    console.log("--------- PROCESSING EXTRACTED TASKS ---------");
    console.log(`Processing ${extractedTasks.length} tasks from natural language input:`, JSON.stringify(extractedTasks));
    ...
    try {
      ...
      console.error('Error creating tasks from natural language:', error);
    }
  };
            ...
            <div className="flex items-center gap-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setIsNaturalLanguageModalOpen(true)}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex items-center gap-2 p-1 h-9 ml-2">
                    {user?.image ? (
                      <img 
                        src={user.image} 
                        alt={user.name || "User"} 
